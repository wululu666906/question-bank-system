...
def refactor():
    ...
    end_node = {
        'data': {
            'outputs': [{'value_selector': [ITERATION_ID, 'output'], 'variable': 'questions'}],
            'title': '结束',
            'type': 'end'
        },
    ...
    edges = [
        ...
        {'id': 'e6', 'source': CODE_RULES_ID, 'target': ITERATION_ID, 'type': 'custom'},
    ...
    # --- 3. Graph Assembly ---
    dsl['workflow']['graph']['nodes'] = [start_node, code_rules, knowledge_node, scenario_node, json_parser, iteration_node, iteration_start, inner_generator, end_node]
    dsl['workflow']['graph']['edges'] = edges

    with open(r'c:\Users\Auraa\Desktop\题库\question_factory_expert_v7_final.yml', 'w', encoding='utf-8') as f:
        yaml.dump(dsl, f, allow_unicode=True, sort_keys=False)

    print("Success: Final Expert DSL v7 generated.")
